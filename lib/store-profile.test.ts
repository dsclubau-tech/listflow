...
test("legacy eBay account resolution prefers the stable login ID", () => {
  ...
  assert.equal(
    resolveLegacyEbayStoreNumber({ loginId: "custom", name: "RK Ecommerce Store" }),
    1,
  );
  assert.equal(
    resolveLegacyEbayStoreNumber({ loginId: "custom", name: "Aussie Walmart" }),
    3,
  );
  assert.equal(
    resolveLegacyEbayStoreNumber({ loginId: "aussie-walmart", name: "My Store" }),
    3,
  );
  assert.equal(
